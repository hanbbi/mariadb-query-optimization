...
수천만 행 규모의 시계열 검침 테이블에서, 월별 사용량 집계 조회가 느려지는 문제를
**진단 → 개선 → 측정**의 순서로 재현한 저장소입니다. 실무에서 수행한 최적화를,
회사 데이터 없이 로컬 더미 데이터로 누구나 재현할 수 있게 구성했습니다.

> 핵심 결과(예시 형식 — 본인 환경 측정값으로 교체): **약 10초 → 약 2초**
> ([results/benchmark.md](results/benchmark.md)에 실측값 기록)
